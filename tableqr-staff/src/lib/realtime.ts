...
export interface OrderStream { readonly error: unknown; readonly isLoading: boolean; readonly orders: readonly StaffOrderDto[]; readonly refetch: () => void }
export function useOrderStream(): OrderStream {
  ...
  return { error: query.error, isLoading: query.isPending, orders, refetch: () => void query.refetch() }
}
