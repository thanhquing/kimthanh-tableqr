...
export class StaffService {
  ...
  async orders(status?: string, since?: string) {
    ...
    return { serverTime: new Date(), orders: orders.map((order) => this.orderDto(order, true)) }
  }
  ...
  async tables() {
    ...
    return { tables: tables.map((table) => { const session = sessions.find((candidate) => candidate.tableId === table.id); const sessionOrders = session ? orders.filter((order) => order.sessionId === session.id) : []; return { id: table.id, code: table.code, displayName: table.displayName, status: session ? 'OCCUPIED' : 'EMPTY', sortOrder: table.sortOrder, session: session ? { id: session.id, openedAt: session.openedAt, totalVnd: sessionOrders.reduce((sum, order) => sum + order.items.reduce((itemSum, item) => itemSum + item.unitPriceVndSnapshot * item.quantity, 0), 0), orderCount: sessionOrders.length, paidAt: session.paidAt } : null } }) }
  }
  async session(id: string) { const session = await this.prisma.tableSession.findUnique({ where: { id }, include: { table: true, orders: { include: { items: true }, orderBy: { sequenceNo: 'asc' } } } }); if (!session) fail(404, 'SESSION_NOT_FOUND', 'Không tìm thấy phiên bàn.'); const orders = session!.orders.map((order) => this.orderDto(order)); return { table: session!.table, session: { id: session!.id, status: session!.status, totalVnd: this.total(orders), paidAt: session!.paidAt }, orders } }
  ...
  private orderDto(order: { id: string; sessionId: string; sequenceNo: number; status: string; createdAt: Date; note: string | null; items: Array<{ nameSnapshot: string; quantity: number; note: string | null; unitPriceVndSnapshot: number }>; table?: { id: string; code: string; displayName: string } }, withTable = false) { const items = order.items.map((item) => ({ ...item, lineTotalVnd: item.unitPriceVndSnapshot * item.quantity })); return { id: order.id, sequenceNo: order.sequenceNo, status: order.status, createdAt: order.createdAt, note: order.note, totalVnd: items.reduce((sum, item) => sum + item.lineTotalVnd, 0), ...(withTable && order.table ? { table: order.table, sessionId: order.sessionId } : {}), items } }
  private total(orders: Array<{ status: string; totalVnd: number }>) { return orders.filter((order) => order.status !== 'CANCELLED').reduce((sum, order) => sum + order.totalVnd, 0) }
}
