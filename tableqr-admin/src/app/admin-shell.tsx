...
export function AdminShell({ children }: { readonly children: ReactNode }) {
  ...
  return <div className={open ? 'admin-app admin-app--open' : 'admin-app'}>
    <aside className="admin-sidebar"><div className="admin-brand"><LayoutGrid size={23} /><strong>Kim Thành</strong><small>{auth?.displayName}</small></div><nav>{links.map(({ icon: Icon, label, to }) => <NavLink key={to} onClick={() => setOpen(false)} to={to}><Icon size={19} />{label}</NavLink>)}</nav><button className="admin-logout" onClick={logout} type="button"><LogOut size={18} />Đăng xuất</button></aside>
