...
export function AdminShell({ children }: { readonly children: ReactNode }) {
  ...
  return <div className={open ? 'admin-app admin-app--open' : 'admin-app'}>
    ...
    <div className="admin-content"><header className="admin-mobile-header"><button aria-label="Mở điều hướng" onClick={() => setOpen((value) => !value)} type="button"><Menu size={22} /></button><strong>{restaurantName}</strong></header><main><BillingBanner />{children}</main></div>
  </div>
}
