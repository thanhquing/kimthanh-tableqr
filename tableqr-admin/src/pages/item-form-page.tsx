...
export function ItemFormPage() {
  ...
  return <form className="admin-form" onSubmit={(event) => void save(event)}>
    ...
    {uploadError ? <p className="admin-menu__error">{uploadError}{lastFile ? <Button onClick={() => void upload(lastFile)} type="button" variant="secondary">Thử lại</Button> : null}</p> : null}
    {form.imageUrl ? <div className="admin-upload__preview"><img alt="Xem trước ảnh món" src={form.imageUrl} /><Button onClick={() => { change('imageUrl', ''); setLastFile(null); setUploadError('') }} type="button" variant="secondary">Bỏ ảnh</Button></div> : null}
